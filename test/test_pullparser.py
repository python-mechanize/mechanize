...
<body>
<p>This is a data <img alt="blah &amp; &#097;"> &amp; that was an entityref \
and this &#097; is
a charref.  <blah foo="bing" blam="wallop">.
<!-- comment blah blah
still a comment , blah and a space at the end__SPACE__
-->
<!rheum>
<?rhaponicum>
<randomtag spam="eggs"/>
...
""".replace('__SPACE__', ' ')  # "
    ...
    def test_encoding(self):
        ...
        # for pc, tolerant in [(_pullparser.PullParser,
        # False)]:#PullParserTests.PARSERS:
...
"http://www.w3.org/TR/html4/strict.dtd"''', None))
        ...
        self.assertEqual(p.get_token(), ("data", "\n", None))
        self.assertEqual(p.get_token(), ("starttag",
                                         "title", [("an", "attr")]))
        ...
        self.assertEqual(p.get_token(), (
            "starttag", "blah", [("foo", "bing"), ("blam", "wallop")]))
        self.assertEqual(p.get_token(), ("data", ".\n", None))
        self.assertEqual(p.get_token(), (
            "comment", " comment blah blah\n"
            "still a comment , blah and a space at the end \n", None))
        self.assertEqual(p.get_token(), ("data", "\n", None))
        decl = 'decl' if tolerant else 'comment'
...
#        sys.exit()
    ...
    def _test_get_text(self, parser_class, tolerant):
        ...
        self.assertEqual(p.get_text(), "\n")
        p.get_token()
        self.assertEqual(p.get_text(), "Title")
        p.get_token()
        self.assertEqual(p.get_text(), "\n")
        p.get_token()
        self.assertEqual(p.get_text(), "\n")
